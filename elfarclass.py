...
class ElFarolBar:
    def __init__(self, num_agents, num_contagious_agents, capacity, threshold, contagious_threshold, contagious_duration, people_memory_weight: float = 0.7, contagious_thresholdNotPresent: float = 1, Use_SIR: bool = False, SIR_AgentsRecoveryTime: int = 0, debugCSV: bool = False): 
    ...
    def chartSave(self, max_line: bool = True, threshold_line: bool = True, cont_threshold_line: bool = True, contNotPres_threshold_line: bool = True):
        arr_x = []
        for i in range(1, len(self.attendance_history) + 1):
            arr_x.append(i)
        if max_line:
        ...
        plt.plot(arr_x, self.present_contagious_history, label="Contagious people level in bar")
        if threshold_line:
            line_threshold = plt.Line2D((0, arr_x[len(arr_x) - 1]), (self.threshold * self.num_agents, self.threshold * self.num_agents), color="green", label=f"Agents treshold: {self.threshold * 100}% -> {self.threshold * self.num_agents}")
            plt.gca().add_line(line_threshold)
        if cont_threshold_line:
            line_threshold = plt.Line2D((0, arr_x[len(arr_x) - 1]), (self.contagious_threshold * self.num_agents, self.contagious_threshold * self.num_agents), color="blue", label=f"Contagious treshold:  {self.contagious_threshold * 100}% -> {self.contagious_threshold * self.num_agents}")
            plt.gca().add_line(line_threshold)
        if contNotPres_threshold_line:
            line_threshold = plt.Line2D((0, arr_x[len(arr_x) - 1]), (self.contagious_thresholdNotPresent * self.num_agents, self.contagious_thresholdNotPresent * self.num_agents), color="lightblue", label=f"Contagious treshold (not present): {self.contagious_thresholdNotPresent * 100}% -> {self.contagious_thresholdNotPresent * self.num_agents}")
            plt.gca().add_line(line_threshold)
        plt.legend(loc="upper left", bbox_to_anchor=(-0.15,1.25), ncol=2)
        baseDir = 'OutputImg/'
        fileAmmount = len(os.listdir(baseDir))
        plt.savefig(baseDir + 'Export_' + str(fileAmmount), bbox_inches='tight')
